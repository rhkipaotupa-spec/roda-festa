...
Estado reconciliado em 04/09/2026 a partir de `main` no commit `5292da7268b134f0a4b822e48c13623073f2da99`.

1. Abrir `C:\Projetos\roda-festa` e confirmar `main`, working tree limpa e `HEAD == origin/main` antes de qualquer nova frente.
2. Tratar `5292da7268b134f0a4b822e48c13623073f2da99` como baseline seguro de retomada atual até existir um novo merge aprovado.
...
21. **P4 — FECHADO:** PR #11 mergeada em `ad6eb282dc83da65c209b96e9fbc0637f35bbb90`. `react-router-dom/react-router` = 7.18.2, `brace-expansion` = 5.0.9 e `nanoid` = 3.3.18; `npm audit` completo e `--omit=dev` = zero vulnerabilidades; security 110/110, full 417/417, lint/build GREEN; CI/Vercel/Production SUCCESS.
22. **P5 — FECHADO:** PR #12 mergeada em `5292da7268b134f0a4b822e48c13623073f2da99`; workflow usa `actions/checkout@v7`, `actions/setup-node@v7`, Node 24 e preserva `validate` como job obrigatório.
23. **Governança `main`:** ruleset `Protect main` id `22214695` ativo na default branch, sem bypass; delete e force push bloqueados; PR obrigatório; status check obrigatório `validate`; approvals = 0; strict up-to-date = off.
24. Toda rota nova ou alterada que aceite ID deve provar posse/escopo no servidor. Para PlanningSession pública, preservar filtro `sessionId + tokenHash`; para Admin, preservar autenticação + autorização server-side.
25. Toda nova tela/botão privilegiado deve ser cruzada com seu endpoint correspondente. Ocultar UI por role não conta como controle de segurança; o backend precisa negar acesso sem privilégio.
26. Toda mudança em migration, adapter, query, listagem, relatório ou exportação deve revisar RLS/grants e o mecanismo de isolamento aplicável. Nova tabela pública deve nascer com RLS habilitado e grants mínimos.
27. Toda atualização deve revisar segredos no diff. Nunca hardcodar chaves, tokens, senhas, secrets, connection strings privilegiadas ou defaults públicos que possam virar segredo real. Configuração sensível ausente deve falhar fechado.
28. Se a mudança afetar frontend/env de build, confirmar que nenhum segredo server-side entra no bundle Vite. Varredura histórica dedicada deve ser repetida quando houver risco novo, evidência de vazamento ou alteração relevante das superfícies cobertas.
29. Mudanças que renderizem input em HTML/e-mail/PDF/template/markdown devem preservar escape/sanitização adequada. `dangerouslySetInnerHTML`, `innerHTML`, HTML bruto, `eval`, `new Function` e URLs controláveis exigem revisão específica.
30. Antes do PR, executar conforme aplicável: `npm run test:security`, testes focados, `npm test`, `npm run lint`, `npm run build`, `git diff --check`, revisão de segredos e revisão específica de auth/ID/input/banco quando essas superfícies forem alteradas.
31. Mudanças de segurança devem ter teste negativo que falhe antes da correção e passe depois sempre que tecnicamente possível.
32. Novas alterações devem sair de `main` limpa em branch isolada, ser agrupadas em lote autocontido e só então abrir PR.
33. Antes de pedir merge: CI verde no SHA exato do head, Vercel/preview verde quando aplicável, diff revisado sem arquivos inesperados, nenhum segredo no diff e documentação reconciliada quando a mudança alterar arquitetura/segurança/DR.
34. Não mergear PR sem aprovação explícita. Após merge aprovado: reconciliar local/remoto, smoke aplicável, documentação e novo snapshot seguro quando a frente exigir.
35. Nunca registrar senha, token, cookie, chave, service role, connection string com segredo, conteúdo de `.env*` ou chave de backup em chat, docs ou Git.
36. Ao encerrar o dia, deixar claro o último commit seguro, estado da branch, gates executados, pendências reais e se existe ou não snapshot final reconciliado.
